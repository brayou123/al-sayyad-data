...
def nearest_value(ds, var):
    ...
    if data.ndim == 2: val = data[ilat, ilon]
    elif data.ndim == 1: val = data[ilat]
    else: val = data[ilat, ilon]
...
def nearest_profile(ds, var):
    ...
    ilon = np.argmin(np.abs(lons - TARGET_LON))
    data = ds[var].values
    if data.ndim == 4: profile = data[0, :, ilat, ilon]
    elif data.ndim == 3: profile = data[:, ilat, ilon]
    else: profile = data
...
if sst and sst > 100: sst = sst - 273.15
...
current_ms  = round(float(np.sqrt(uo**2 + vo**2)), 3) if uo and vo else None
current_kn  = round(current_ms * 1.944, 2) if current_ms else None
current_dir = round(float(np.degrees(np.arctan2(uo, vo))) % 360, 1) if uo and vo else None
...
if valid.sum() >= 2:
    ...
    # ignore surface warming — start from index 2
    grad[:2] = 0
    idx = np.argmax(grad)
    thermo_depth    = round(float((d_valid[idx] + d_valid[idx+1]) / 2), 1)
    thermo_gradient = round(float(grad[idx]), 2)

print()
print("=" * 50)
print(f"  POINT DATA — {TARGET_DATE}")
print(f"  {TARGET_LAT}°N, {TARGET_LON}°E")
print("=" * 50)
print(f"  🌡️  Temperature : {round(sst,2) if sst else '—'} °C")
...
print(f"  👁️  Secchi      : {secchi if secchi else '—'} m")
print(f"  🌊  Current     : {current_kn if current_kn else '—'} kn")
print(f"  🧭  Direction   : {current_dir if current_dir else '—'} °")
print(f"  📏  Thermocline : {thermo_depth if thermo_depth else '—'} m")
print(f"  📐  Gradient    : {thermo_gradient if thermo_gradient else '—'} °C/m")
print()
